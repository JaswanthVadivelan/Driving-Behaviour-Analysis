...
from src.scorer import TripScorer
...
with col_ds:
    ...
    available_files = [f for f in os.listdir(datasets_dir) if f.lower().endswith(".csv")]
    selected_dataset = st.selectbox("Select dataset from folder (optional)", options=[""] + available_files)
...
if run_scoring:
    if not vehicle_id:
        ...
        if selected_dataset:
            input_df = pd.read_csv(os.path.join(datasets_dir, selected_dataset))
        else:
            if save_upload and uploaded_file is not None:
                with open(os.path.join(datasets_dir, uploaded_file.name), "wb") as f:
